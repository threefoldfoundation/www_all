...
> can just be installed in your hosted [3Bot](threefold__3bot_def)
...
- **package.toml** is where the package information is defined such as its name, ports, and type of content for example static website.<br />
 ...
 websocket = true
 ```
 *Detailed types of `nginx` locations that could be defined inside `package.toml` are documented [here](sdk__locations.md).
...
- **package.py** manages the lifecycle of the package.
 ...
 - If additional functionalities want to be added during install,uninstall,start of the package in [3Bot](threefold__3bot_def) server they could be redifined in this file.
...
- **actors**
 ...
 Example
  ...
  Actor = HelloActor
  ```
 - To know more see [actors](sdk__actors.md)
...
- **chats**
 ...
 Example
  ...
  chat = HelloChatflow
  ```
 - To know more see [chatflows](sdk__develop_chatflow.md)
...
## Package manager
- `Packages` is an actor on [3Bot](threefold__3bot_def) under the `admin` package, any user with admin rights could call this actor to remotely instruct the [3Bot](threefold__3bot_def) to install/remove/start/stop a package from [3Bot](threefold__3bot_def) server
...
## Adding new packages
Adding a package could easily be done either through the [3Bot](threefold__3bot_def) server directly or using the package manager actor mentioned earlier


### Add the package to the 3Bot server

A package could be added directly to the [3Bot](threefold__3bot_def) server as follows
```python
...
This also applies to deleting and installing packages directly using the [3Bot](threefold__3bot_def) server.

### Add the package using package manager actor

After starting the server with the recommended way, the package created could be added using the packages actors which calls the package manager of the [3Bot](threefold__3bot_def) server. The actor could be called as follows:

```
j.clients.gedis.threebot.actors.admin_packages.add_package("<path_to_package>")
```
where [3Bot](threefold__3bot_def) is the gedis instance name for the [3Bot](threefold__3bot_def) started
