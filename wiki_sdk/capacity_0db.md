...
0-DB server is running on [Zero-OS](threefold__zos) node and exposed on the [Zero-OS](threefold__zos) nodes over public IPv6 addresses. The address of a specific 0-DB remains constant over time. This means even in the event a node reboots, the 0-DB will automatically start and have the same IP address.
