...
The TF Grid has a number of online resources to create [3Node](threefold__3node)s, to find [3Node](threefold__3node)s with capacity and to bring forward and upload container images into Flists. All of these resources are available to use, are run on the grid but also are available as open-source to create your own private hub.

### Create a 3Node
Anyone could creates [3Node](threefold__3node)s. The process is very simple and requires you to put forward and dedicate hardware for the [3Node](threefold__3node)s and get a bootloader at [dev.bootstrap.grid.tf](https://dev.bootstrap.grid.tf/). Mode information about the process of listing your farm and getting a `farmID` could be found here 
...
The TF Grid has an explorer which represents all of the [3Node](threefold__3node)s that form the capacity grid. The TF Grid is ever-expanding and at the time of writing we are in the middle of migrating nodes from grid v1.0 to v2.0. The explorer provide some basic number on total capacity available, geographics spread and actual workloads deployed on the grid. 
...
The Flist principle is explained in the introduction architecture section ([here](sdk__architecture_flist.md).
<!-- ![](img/hub_startpage.png) -->

<img src="img/hub_startpage.png" width="600" alt="Docker converter">

Access to this hub is restricted to people that have a ThreeFold Connect identity. There is a description in the "getting started" section on how to get one of these [here](sdk__threefold_connect_install.md).
...
![](sdk__docker_converter.png  )
![](sdk__homemade_flist.png  )
-->
